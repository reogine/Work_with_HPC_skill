...
**Official package:** The PyPI package is `hpc-skill`. 
The CLI command is also `hpc-skill`.

### Step 1 — install the package:

```bash
# Recommended (isolated env):
uv tool install hpc-skill

# Alternatives:
pipx install hpc-skill
pip install hpc-skill
```
...
```bash
hpc-skill install --project
```
Project-scoped installs write under the current directory (specifically `.agents/skills/`) and print a hint for files that can be committed. 

*Note: If `hpc-skill: command not found` appears after `pip install`, ensure `~/.local/bin` is on your PATH.*
